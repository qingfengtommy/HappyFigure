...
# Set by run_once.py (or any caller) before invoking the parallel pipeline.
...
class FigurePipelineState(TypedDict, total=False):
    ...
    planner_mode_hint: str  # planner instruction suffix controlled by run_once mode
...
def _compact_experiment_group(group: dict) -> dict:
    ...
    compact["statistics"] = _truncate_for_prompt(group.get("statistics") or "", _STATISTICS_CHARS_MAX, f"{name}.statistics")
...
def _apply_route_overrides(per_experiment_routes: dict[str, dict], state: FigurePipelineState) -> dict[str, dict]:
    """Apply mode-driven route overrides from run_once orchestration."""
        ...
        try:
            ...
            if fp.suffix.lower() == ".md":
                ...
                for section_name, section_df in section_dfs:
                    ...
                    virtual_rel = (
                        f"{rel} [Section: {section_name}]"
                        if len(section_dfs) > 1
                        else str(rel)
                    )
                    ...
                    sec_sample_lines = "\n".join(
                        "  " + line for line in sec_sample_text.splitlines()
                    )
                    ...
                    for c in sec_num_cols:
                        ...
                        sec_range_lines.append(
                            f"    {c}:  {col_min:.6g} / {col_mean:.6g} / {col_max:.6g}"
                        )
    ...
    for li in level_info:
        ...
        if li["children_counts"]:
            ...
            if len(counts_set) == 1:
                ...
                lines.append(f"  Children per entry: {min(li['children_counts'])}–{max(li['children_counts'])} (variable)")
        lines.append("")
...
def pre_data_explorer_node(state: FigurePipelineState) -> FigurePipelineState:
    ...
    try:
        ...
        for pattern in ("**/*.csv", "**/*.tsv", "**/*.json", "**/*.md", "**/*.npy", "**/*.npz", "**/*.pkl", "**/*.pt", "**/*.ckpt"):
...
def load_input_node(state: FigurePipelineState) -> FigurePipelineState:
    ...
    if not results_path.is_absolute():
        results_path = Path.cwd() / results_dir
    try:
        results_dir_rel = str(results_path.relative_to(REPO_ROOT)) if results_path.is_relative_to(REPO_ROOT) else str(results_path)
    except ValueError:
...
def data_processor_node(state: FigurePipelineState) -> FigurePipelineState:
    ...
    try:
        ...
        result = _retry_llm_call(lambda: run_prompt_with_tools(
            "code",
            user_text.strip(),
...
def _scan_processed_data(processed_path: Path) -> tuple:
    ...
    try:
        processed_dir_rel = str(processed_path.relative_to(REPO_ROOT)) if processed_path.is_relative_to(REPO_ROOT) else str(processed_path)
    except ValueError:
...
def execute_data_processing_node(state: FigurePipelineState) -> FigurePipelineState:
    ...
    def _scan_and_build_output(
        ...
        *,
        ...
        has_real_data = any(
            f
            for g in _DATA_GLOBS
            for f in processed_path.rglob(g)
        )
    ...
    if mode == "reuse":
        ...
        if out is not None:
            logger.info("Data processing skipped (reuse): %d experiment group(s).", len(out.get('experiment_groups') or []))
            return out
    ...
    for attempt in range(1, MAX_DATA_PROCESSING_RETRIES + 1):
        ...
        if returncode == 0:
            ...
            if processed_path:
                ...
                if out is not None:
                    logger.info("Data processing complete: %d experiment group(s).", len(out.get('experiment_groups') or []))
                    return out
                stderr = err
        ...
        try:
            ...
            if fixed_code.strip():
                ...
                if run_dir:
                    (Path(run_dir) / f"data_processing_code_v{attempt + 1}.py").write_text(
                        code, encoding="utf-8"
                    )
        except Exception as e:
...
def route_figures_node(state: FigurePipelineState) -> FigurePipelineState:
    ...
    if run_dir:
        safe_routes = {
            k: {ik: iv for ik, iv in v.items() if not str(ik).startswith("_")}
            for k, v in per_experiment_routes.items()
        }
    ...
    if groups and (
        len(groups) != len(state.get("experiment_groups") or [])
        or first_name != state.get("current_experiment_name")
    ):
        out["experiment_groups"] = groups
...
def _build_style_few_shot_messages(few_shots: List[dict]) -> list[dict]:
    ...
    for i, shot in enumerate(few_shots, 1):
        ...
        messages.append({
            "role": "assistant",
            "content": [{"type": "output_text", "text": f"Noted. I will use this as a style reference (example {i})."}],
        })
...
def code_agent_node(state: FigurePipelineState) -> FigurePipelineState:
    ...
    if not code:
        try:
            code = _retry_llm_call(lambda: run_prompt(
                "code",
                user_text.strip(),
                system_prompt=system_text.rstrip(),
                image_base64=image_to_send,
...
def execute_code_node(state: FigurePipelineState) -> FigurePipelineState:
    ...
    try:
        ...
        if not figure_path:
            ...
            candidates = [
                p for p in _glob.glob(str(figures_dir / f"*{safe_exp}*.png"))
...
def critic_node(state: FigurePipelineState) -> FigurePipelineState:
    ...
    try:
        ...
        if not response:
            response = run_prompt(
                "chat", user_text, system_prompt=system_text, image_base64=image_data_url
            ).strip()
    ...
    iteration = state.get("iteration") or 1
    _artifact_dir(state).joinpath(f"critic_feedback_iter{iteration}.md").write_text(
...
def plan_all_experiments_node(state: FigurePipelineState) -> FigurePipelineState:
    ...
    if invalid_routes:
        _status_print(f"WARNING: Routing metadata missing/invalid for {len(invalid_routes)} spec(s). Retrying planner once.")
...
def execute_experiment_iteration(exp_state: dict, iteration: int) -> dict:
    ...
    _report_progress(name, "critic_done", iteration,
...
def _beam_search_experiment(
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=min(total_cands, 4)) as executor:
        futures = [
            executor.submit(execute_experiment_iteration, cs, 1)
    ...
    _report_progress(
        name, "beam_rank", 1, survivors=beam_width,
    ...
    if "ACCEPT" in (survivors[0].get("critic_verdict") or "").upper():
        beam_log.append(
            f"## Early stop — best candidate ACCEPTED "
            f"with score {survivors[0].get('critic_score', 0):.1f}"
        )
    else:
        ...
        for iteration in range(2, beam_iterations + 1):
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=min(total_cands, 4)) as executor:
                futures = [
                    executor.submit(execute_experiment_iteration, cs, iteration)
                    for cs in candidate_states
                ]
                candidates = [f.result() for f in futures]
            ...
            _report_progress(
                name, "beam_rank", iteration, survivors=beam_width,
                best_score=survivors[0].get("critic_score", 0) if survivors else 0,
            )
    ...
    beam_log.append(
        f"  Winner: {best_tag} (score={best_score:.1f}, "
        f"verdict={best.get('critic_verdict', '?')})"
    )
